...
library HookMiner {
    ...
    function find(
        ...
        for (uint256 salt; salt < MAX_LOOP; salt++) {
            hookAddress = computeAddress(deployer, account, salt, creationCodeWithArgs);
    ...
    function computeAddress(address deployer, address account, uint256 salt, bytes memory creationCodeWithArgs)
        internal
        pure
